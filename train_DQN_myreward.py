...
def load_agent_submission(submission_dir: Path):
    ...
    import agent_myDQN48 as agent_submission
...
def train(agent_spec, simulator_cfg_file, gym_cfg, metric_period):
    ...
    for e in range(args.episodes):
        ...
        while i < args.steps:
            if i % args.action_interval == 0:
                ...
                for key, val in observations.items():
                    ...
                    val = val[1:]
                    while len(val) < agent.ob_length//2:
                        val.append(0)
        ...
        if all_loss_i>0:
            print("episode:{}/{},  average_loss : {:.8f}".format(e, args.episodes ,all_loss/all_loss_i))
        ...
        time22 = time.time()
        print("from {} to {}   ---{:.4f} s".format(time1,time2,time22-time11))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', type=str, default="model/dqn48",
                        help='directory in which model should be saved')
    parser.add_argument('--log_dir', type=str, default="cmd_log/dqn48", help='directory in which logs should be saved')
