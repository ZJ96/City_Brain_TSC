...
def load_agent_submission(submission_dir: Path):
    ...
    import agent_PPO2 as agent_submission
...
def train(agent_spec, simulator_cfg_file, gym_cfg, metric_period):
    ...
    for e in range(args.episodes):
        print("----------------------------------------------------{}/{}".format(e+1, args.episodes))
        last_obs = env.reset()
        #print(last_obs)
        episodes_rewards = {}
        ...
        while i < args.steps:
            if i % args.action_interval == 0:
                ...
                for key, val in observations.items():
                    ...
                    if observations_agent_id not in all_data_buffer.keys():
                        all_data_buffer[observations_agent_id] = []
                    val = val[1:]
                    while len(val) < 24:
                        val.append(0)
                ...
                actions, actions_prob = agent.act_(observations_for_agent)
                #actions = {}，且   {agentid: action} 如{1414141:0,1341414:7,....}
                rewards_list = {}
                ...
                # Remember  state, action, logprob, reward,done  into memory buffer.
                for agent_id in agent_id_list:
                    #trans = Transition(observations_for_agent[agent_id]['lane'], actions[agent_id],
                    #                   actions_prob[agent_id], rewards[agent_id], new_observations_for_agent[agent_id]['lane'])

                    #all_data_buffer[agent_id].append(trans)
                    all_data_buffer[agent_id].append((observations_for_agent[agent_id]['lane'],actions[agent_id],
                                                      actions_prob[agent_id], rewards[agent_id]))
        ...
        print("------------    update policy    -------")
        for agent_id in agent_id_list:
            every_agent_buffer = all_data_buffer[agent_id]
            for aa,bb,cc,dd in every_agent_buffer:
                #print(aa,bb,cc,dd)
                agent.remember(aa,bb,cc,dd,False)
            all_loss += agent.update()

        avg_loss = all_loss/len(agent_id_list)
        print("episode:{}/{}, average loss:{}".format(e, args.episodes, avg_loss))
        ...
        all_agent_mean_reward = all_agent_reward/episodes_decision_num
        print("episode:{}/{}, agent_id_list_len = {},    all_agent_mean_reward : {:.8f}".format(e, args.episodes, len(agent_id_list),all_agent_mean_reward))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--steps', type=int, default=10, help='number of steps')  #360
    ...
    parser.add_argument('--log_dir', type=str, default="cmd_log/ppo", help='directory in which logs should be saved')
