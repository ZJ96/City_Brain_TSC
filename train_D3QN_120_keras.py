...
action_to_way = [(1,7),(2,8),(4,10),(5,11),(1,2),(4,5),(7,8),(10,11)]
def get_rewards(action,new_ob): #new_ob = 0-24
    wait_car = 0
    pass_car = 0
    for i in range(1,13):
        if new_ob[i]==-1:
    ...
    for i in range(13,25):
        ...
        #all_car+=new_ob[i]
        pass_car+=new_ob[i]
    #a,b = action_to_way[action][0],action_to_way[action][1]
    #action_car = new_ob[12+a]+new_ob[12+b]
    return wait_car,pass_car#,action_car

def get_around_inter(roads:dict, agents:dict):
    connect_intersections = {}
    for agent_id in agents.keys():
        connect_intersections[agent_id] = []
        tmp_agent = agents[agent_id]
        for i in range(4):
            tmp_road = tmp_agent[i]
            if tmp_road == -1:
                connect_intersections[agent_id].append(-1)
                continue
            tmp = roads[tmp_road]
            if tmp['start_inter']!=agent_id:
                connect_intersections[agent_id].append(tmp['start_inter'])
            else:
                connect_intersections[agent_id].append(tmp['end_inter'])
    return connect_intersections

def get_all_states(connect_intersections:dict,observations_for_agent:dict):
    zero = [0 for i in range(24)]
    new_state = {}
    for agent_id in observations_for_agent.keys():
        new_state[agent_id] = copy(observations_for_agent[agent_id]['lane'])
        for connet_inter in connect_intersections[agent_id]:
            if connet_inter == -1 or connet_inter not in observations_for_agent.keys():
                new_state[agent_id] += copy(zero)
            else:
                new_state[agent_id] += copy(observations_for_agent[connet_inter]['lane'])
...
def load_agent_submission(submission_dir: Path):
    ...
    import agent_D3QN_120_keras as agent_submission
    #import agent_MP4 as agent_submission
...
def train(agent_spec, simulator_cfg_file, gym_cfg, metric_period):
    ...
    connect_intersections = get_around_inter(roads,agents)
    #print(connect_intersections)
    ...
    for e in range(args.episodes):
        ...
        time11 = time.time()
        print("----------------------------{}/{}---------start time {}".format(e, args.episodes, time1))
        last_obs = env.reset()
        episodes_rewards = {}
        ...
        while i < args.steps:
            if i % args.action_interval == 0:
                ...
                old_state = get_all_states(connect_intersections,observations_for_agent)
                # Get the action, note that we use act_() for training.
                actions = agent.act_(old_state)
                ...
                for _ in range(args.action_interval):
                    ...
                    for agent_id in agent_id_list:
                        ...
                        if agent_id in rewards_list:
                            ...
                            rewards_list[agent_id] = pressure
                        # wait_car,pass_car = get_rewards(actions[agent_id],lane_vehicle)
                        # if wait_car+pass_car==0:
                        #     pressure = pass_car-wait_car   #+action_car
                        # else:
                        #     pressure = (pass_car-wait_car)*(pass_car/(wait_car+pass_car))
                        # pressure = pressure/args.action_interval
                        # if agent_id in rewards_list:
                        #     rewards_list[agent_id] += pressure
                        # else:
                        #     rewards_list[agent_id] = pressure
                ...
                new_state = get_all_states(connect_intersections, new_observations_for_agent)

                # Remember (state, action, reward, next_state) into memory buffer.
                for agent_id in agent_id_list:
                    agent.remember(old_state[agent_id], actions[agent_id], rewards[agent_id],
                                   new_state[agent_id])
            ...
            if all(dones.values()):
                break
        if (e+1) % args.save_rate == 0:
            if not os.path.exists(args.save_dir):
                os.makedirs(args.save_dir)
            agent.save_model(args.save_dir, e+1)
        if episodes_loss_num>0:
            avg_loss = episodes_loss/episodes_loss_num
            print("episode:{}/{}, average loss :{:.8f}".format(e, args.episodes, avg_loss))

        logger.info(
            "episode:{}/{}, average travel time:{:.8f}".format(e, args.episodes, env.eng.get_average_travel_time()))
        all_agent_reward = 0
        for agent_id in agent_id_list:
            all_agent_reward += episodes_rewards[agent_id]
            # logger.info(
            #     "agent:{}, mean_episode_reward:{}".format(agent_id,
            #                                               episodes_rewards[agent_id] / episodes_decision_num))
        all_agent_mean_reward = all_agent_reward/len(agent_id_list)/episodes_decision_num
        print("episode:{}/{},     agent_id_list_len = {},     all_agent_mean_reward : {:.8f}".
              format(e, args.episodes, len(agent_id_list),all_agent_mean_reward))
        time2 = time.strftime("%Y%m%d_%H:%M:%S")
        time22 = time.time()
        print("############################ from {} to {} cost {:.4f} minutes".format(time1,time2,(time22-time11)/60))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', type=str, default="model/dqn_keras",
                        help='directory in which model should be saved')
    parser.add_argument('--log_dir', type=str, default="cmd_log/dqn_keras", help='directory in which logs should be saved')
